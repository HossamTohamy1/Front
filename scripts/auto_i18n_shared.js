...
const allFiles = findArabicFiles(path.join(__dirname, '../src/app/shared'));
...
Array.from(uniqueArabicStrings).sort((a,b) => b.length - a.length).forEach(arStr => {
  if (existingArToKey[arStr]) {
    ...
    const newKey = `SHARED.AUTO_STR_${autoCounter++}`;
    newMappings[arStr] = newKey;
    
    if (!enJson.SHARED) enJson.SHARED = {};
    if (!arJson.SHARED) arJson.SHARED = {};
    
    enJson.SHARED[`AUTO_STR_${autoCounter - 1}`] = `[EN] ${arStr}`;
    arJson.SHARED[`AUTO_STR_${autoCounter - 1}`] = arStr;
  }
});
